...
It looks for __tasks.json__ in the __.vscode__ folder, then it locates the task labeled as `"group": {"kind": "build, "isDefault": true}`.
- If there is no `isDefault: true` task, VSCode will prompt you to select a task from those marked with `"kind": "build"`.
...
It uses the `launch.json` Configuration: VSCode reads the configuration in the `.vscode/launch.json` file, which defines how your code should be executed. This file specifies details such as the environment, runtime arguments, and other settings.
- It uses the first defined configuration in launch.json, or the configuration you’ve marked as default or selected previously.
...
preLaunchTask: The task to run before starting the debugger. This can be a task label.
